...
| **2. Backend & Security** | Orphan Endpoints | **FAIL** | `/api/families`, `/api/journey_progress`, `/api/courses` exist in backend router but are never called by `apiClient` in `web/src` (E3). | P3 | Unused API surface area. |
...
| **8. Observability** | Proper Logger Usage | **FAIL** | `pkg/game/gift/catalog.go:68,75,99` uses standard library `log.Printf` instead of the injected `observability.Logger` (E3). | P2 | Bypasses structured logging and metrics. |
