...
// Real authenticated production smoke for 2026-09-20 + 2026-09-21 tasks
// from Migration 090. Task IDs are resolved at runtime by exact
...
// No redeem/claims. No purchase is made: task 2/5 evidence is fictional
// clearly-marked test data; task 3 photo is a disposable synthetic test
...
const TEST_EXPLORER = `Smoke Sep20-21 ${ts}`;
...
const QUIZ_CORRECT = { q1: 'A', q2: 'A', q3: 'A', q4: 'A' };
...
async function run() {
  console.log('====================================================');
  console.log('REAL AUTHENTICATED PROD SMOKE — 2026-09-20/21 (090)');
  ...
  const r20 = await dbGet('odyssey_tasks?active_date=eq.2026-09-20&is_active=eq.true&select=id,title,step_order&order=step_order.asc');
  const r21 = await dbGet('odyssey_tasks?active_date=eq.2026-09-21&is_active=eq.true&select=id,title,step_order&order=step_order.asc');
  if (r20.length !== 3 || JSON.stringify(r20.map((t) => t.title)) !== JSON.stringify(TITLES_20)) {
    bad(`09-20 task set wrong: ${JSON.stringify(r20.map((t) => t.title))}`); process.exit(1);
  }
  if (r21.length !== 3 || JSON.stringify(r21.map((t) => t.title)) !== JSON.stringify(TITLES_21)) {
    bad(`09-21 task set wrong: ${JSON.stringify(r21.map((t) => t.title))}`); process.exit(1);
  }
  const [T1, T2, T3] = r20.map((t) => t.id);
  const [T4, T5, T6] = r21.map((t) => t.id);
  console.log(`   T1 MINI_GAME=${T1} T2 TEXT=${T2} T3 PHOTO=${T3} T4 QUIZ=${T4} T5 TEXT=${T5} T6 VIDEO=${T6}`);
  ...
  if (t[T2].json?.task_type === 'TEXT_RESPONSE' && c2cfg.minimum_characters === 150 && c2cfg.maximum_characters === 2000 && /Kemungkinan penyebab/.test(c2cfg.prompt || '')) ok('T2 TEXT bounds 150/2000 + root-cause prompt (DB config)');
  ...
  if (t[T3].json?.task_type === 'PHOTO_UPLOAD' && c3cfg.max_files === 1 && c3cfg.camera_only === false && /Temuan/.test(c3cfg.instruction || '')) ok('T3 PHOTO max_files=1 camera_only=false + 4-part note (DB config)');
  ...
  if (t[T5].json?.task_type === 'TEXT_RESPONSE' && c5cfg.minimum_characters === 150 && c5cfg.maximum_characters === 2000 && /Cara baru usulmu/.test(c5cfg.prompt || '')) ok('T5 TEXT bounds 150/2000 + improvement prompt (DB config)');
  ...
  if (t[T6].json?.task_type === 'VIDEO' && c6cfg.recording?.enabled === true && c6cfg.recording?.max_duration_seconds === 60 && /DILARANG membaca/.test(c6cfg.recording?.instruction || '')) ok('T6 VIDEO recording enabled max 60s + no-reading instruction (DB config)');
  ...
  console.log('\n[4] Task T1 decision scenario (invalid rejected, spoof ignored, valid APPROVED)');
  await sleep(3000);
  ...
  // --- 5. Task T2: min-length validation + valid submit (PENDING) ---
  console.log('\n[5] Task T2 root-cause analysis (validation + PENDING submit)');
  await sleep(3000);
  const short2 = await prodApi(`/api/tasks/${T2}/submit`, { method: 'POST', token, body: { payload: { text: 'SMOKE-TEST pendek', submitted_at: new Date().toISOString() } } });
  if (short2.status === 200 && short2.json?.success === true) bad('T2 accepted too-short text — min-length validation BYPASSED');
  else if (/minimal 150/.test(short2.text)) ok('T2 server rejected short text with min-150 message (DB-configured validation)');
  else bad(`T2 short-text rejection unclear: ${short2.status} ${short2.text.slice(0, 160)}`);
  const text2 = 'SMOKE-TEST DATA (fiktif, akan dihapus). 1) Masalah: contoh masalah uji — kran contoh menetes terus contoh. 2) Penyebab: (a) contoh penyebab uji pertama — seal contoh aus contoh; (b) contoh penyebab uji kedua — putaran contoh kurang kencang contoh. 3) Alasan: contoh alasan uji — tetesan contoh berhenti saat ditekan lebih kencang contoh. 4) Solusi: contoh solusi uji — ganti seal contoh atau putar sampai rapat contoh. 5) Utama: contoh pilihan uji — ganti seal contoh karena paling tahan lama contoh.';
  console.log(`       evidence len = ${text2.length} (bounds 150/2000)`);
  await sleep(3000);
  const s2 = await prodApi(`/api/tasks/${T2}/submit`, { method: 'POST', token, body: { payload: { text: text2, submitted_at: new Date().toISOString() } } });
  ...
  else { manifest.submissions[T2] = rows2[0].id; ok(`submission_id = ${rows2[0].id} (PENDING, text present: ${!!rows2[0].payload?.text})`); }

  // --- 6. Task T3: harmless observation photo + 4-part note (PENDING) ---
  console.log('\n[6] Task T3 overlooked-thing photo (upload + PENDING submit)');
  ...
  form3.append('file', new Blob([pngBytes], { type: 'image/png' }), 'SMOKE-TEST-abaikan.png');
  ...
  if (up3.status !== 200 || !up3.json?.file_url) { bad(`T3 upload failed: ${up3.status} ${up3.text.slice(0, 160)}`); }
    ...
    await sleep(3000);
    const note3 = 'SMOKE-TEST (data uji, akan dihapus). (a) Temuan: contoh temuan uji — cat contoh mengelupas di sudut contoh. (b) Penjelasan: contoh penjelasan uji — lembap contoh membuat cat contoh terkelupas contoh. (c) Kenapa penting: contoh alasan uji — dibiarkan contoh merembet dan merusak tembok contoh. (d) Usulan: contoh usulan uji — amplas lalu cat ulang contoh oleh penghuni contoh.';
  ...
  console.log('\n[7] Task T4 reasoning quiz (invalid rejected, valid APPROVED)');
  await sleep(3000);
  const wrong4 = await prodApi(`/api/tasks/${T4}/submit`, { method: 'POST', token, body: { answers: { q1: 'B', q2: 'C', q3: 'B', q4: 'B' } } });
  ...
  console.log('\n[8] Task T5 practical-improvement text (validation + PENDING submit)');
  await sleep(3000);
  ...
  const text5 = 'SMOKE-TEST DATA (fiktif, akan dihapus). 1) Aktivitas: contoh aktivitas uji — merapikan kamar contoh. 2) Cara sekarang: contoh cara uji — bereskan sekaligus seminggu sekali contoh sampai menumpuk contoh. 3) Masalah: contoh masalah uji — lama contoh dan melelahkan contoh. 4) Cara baru: contoh cara uji — cicil 5 menit tiap pulang contoh per area contoh. 5) Alasan: contoh alasan uji — hemat waktu contoh dan kamar contoh selalu rapi contoh.';
  ...
  console.log('\n[9] Task T6 teach-from-memory video (upload + PENDING submit)');
  const ftyp = Buffer.from([
    0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6f, 0x6d,
    0x00, 0x00, 0x00, 0x00, 0x69, 0x73, 0x6f, 0x6d, 0x69, 0x73, 0x6f, 0x32,
  ]);
  const form6 = new FormData();
  form6.append('file', new Blob([ftyp], { type: 'video/mp4' }), 'SMOKE-TEST-jelaskan.mp4');
  ...
  if (up6.status !== 200 || !up6.json?.file_url) { bad(`T6 upload failed: ${up6.status} ${up6.text.slice(0, 160)}`); }
    ...
    const s6 = await prodApi(`/api/tasks/${T6}/submit`, {
      method: 'POST', token,
      body: { payload: { file_url: up6.json.file_url, file_name: up6.json.file_name, file_size: up6.json.file_size, duration_seconds: 45, mime_type: 'video/mp4', note: 'SMOKE-TEST: video uji disposable (bukan penjelasan asli), mohon abaikan', captured_at: new Date().toISOString() } },
    });
  ...
  const t920 = await dbGet('odyssey_tasks?active_date=eq.2026-09-20&is_active=eq.true&select=id,title&order=step_order.asc');
  const t921 = await dbGet('odyssey_tasks?active_date=eq.2026-09-21&is_active=eq.true&select=id,title&order=step_order.asc');
  if (t920.length === 3 && JSON.stringify(t920.map((x) => x.title)) === JSON.stringify(TITLES_20)) ok(`09-20 intact (exactly 3 active: ${t920.map((x) => x.id).join(',')})`);
  else bad(`09-20 tasks changed: ${JSON.stringify(t920)}`);
  if (t921.length === 3 && JSON.stringify(t921.map((x) => x.title)) === JSON.stringify(TITLES_21)) ok(`09-21 intact (exactly 3 active: ${t921.map((x) => x.id).join(',')})`);
  else bad(`09-21 tasks changed: ${JSON.stringify(t921)}`);
