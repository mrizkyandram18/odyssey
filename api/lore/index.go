...
import (
	...
	"odyssey/pkg/auth"
	"odyssey/pkg/game"
	gameLore "odyssey/pkg/game/lore"
	"odyssey/pkg/shared"
)
...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	rest := strings.TrimPrefix(path, "api/lore")
...
func handleSummary(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims) {
	summary, err := handler.GetSummary(r.Context(), claims.CrewID)
	if err != nil {
		shared.WriteJSONError(w, "failed to get lore summary", http.StatusInternalServerError)
		return
	}
...
func handleUnlocked(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims) {
	result, err := handler.ListUnlocked(r.Context(), claims.CrewID)
	if err != nil {
		shared.WriteJSONError(w, "failed to list unlocked lore", http.StatusInternalServerError)
		return
	}
...
func handleUnlocks(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims) {
	result, err := handler.ListUnlocks(r.Context(), claims.CrewID)
	if err != nil {
		shared.WriteJSONError(w, "failed to list lore unlocks", http.StatusInternalServerError)
		return
	}
	shared.WriteJSON(w, http.StatusOK, result)
}
