...
type Service interface {
	ListChests(ctx context.Context, uid string) ([]game.Chest, error)
	GetChest(ctx context.Context, chestID int64, uid string) (*game.Chest, error)
...
func handleGet(w http.ResponseWriter, r *http.Request) {
	...
	path := strings.TrimPrefix(r.URL.Path, "/api/chests")
	path = strings.TrimPrefix(path, "/")
	if path == "" || path == " " {
		chests, err := svc.ListChests(r.Context(), claims.UID)
		if err != nil {
			shared.WriteJSONError(w, "failed to load chests", http.StatusInternalServerError)
			return
		}
		views := make([]gamechest.ChestView, 0, len(chests))
		for i := range chests {
			views = append(views, gamechest.ToChestView(chests[i]))
		}
