...
func TestDailyTurnStore_ListDailyTurns_Success(t *testing.T) {
	...
	data, _ := json.Marshal([]DailyMission{
		{
			ID:        1,
			UID:       "user-1",
			Date:      "2026-08-01",
...
func TestDailyTurnStore_CreateDailyTurn(t *testing.T) {
	now := time.Date(2026, 8, 3, 12, 0, 0, 0, time.UTC)
	data, _ := json.Marshal([]DailyMission{
		{ID: 1, UID: "user-1", Date: "2026-08-03", MissionSlug: "morning-light", Completed: false, CreatedAt: now},
	})
	store := NewDailyTurnStore(&mockSupabaseClient{data: data})
	dt := &game.DailyMission{
		UID:       "user-1",
		Date:      "2026-08-03",
		MissionSlug: "morning-light",
	}
	...
	if result.MissionSlug != "morning-light" {
		t.Errorf("expected slug morning-light, got %s", result.MissionSlug)
	}
}
