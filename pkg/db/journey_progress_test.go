...
func TestRealmProgressStore_GetRealmProgress_Success(t *testing.T) {
	...
	data, _ := json.Marshal([]JourneyProgress{
		{
			FamilyID:         "crew-1",
			Journey:          "forest",
			Status:         "ACTIVE",
	...
	if rp.Journey != "forest" {
		t.Errorf("expected journey forest, got %s", rp.Journey)
	}
	if rp.Progress != 3 {
...
func TestRealmProgressStore_CreateRealmProgress(t *testing.T) {
	...
	store := NewRealmProgressStore(&mockSupabaseClient{data: data})
	rp, err := store.CreateRealmProgress(context.Background(), &game.JourneyProgress{
		FamilyID:         "crew-1",
		Journey:          "cave",
		Status:         "ACTIVE",
		Progress:       0,
	...
	if rp.Journey != "cave" {
		t.Errorf("expected journey cave, got %s", rp.Journey)
	}
	if rp.Status != "ACTIVE" {
...
func TestRealmProgressStore_CreateRealmProgress_Error(t *testing.T) {
	...
	_, err := store.CreateRealmProgress(context.Background(), &game.JourneyProgress{
		FamilyID: "crew-1",
		Journey:  "cave",
	})
	if err == nil {
...
func TestRealmProgressStore_ListByCrew(t *testing.T) {
	now := time.Date(2026, 8, 3, 12, 0, 0, 0, time.UTC)
	data, _ := json.Marshal([]JourneyProgress{
		{FamilyID: "crew-1", Journey: "forest", Status: "ACTIVE", Progress: 1, UpdatedAt: now},
		{FamilyID: "crew-1", Journey: "cave", Status: "LOCKED", Progress: 0, UpdatedAt: now},
	})
	store := NewRealmProgressStore(&mockSupabaseClient{data: data})
	...
	if progress[0].Journey != "forest" {
		t.Errorf("expected first journey forest, got %s", progress[0].Journey)
	}
}
