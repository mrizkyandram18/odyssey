...
func newMockChapterProgressStore() *mockChapterProgressStore {
	return &mockChapterProgressStore{
		progress: make(map[string]*game.CourseProgress),
	}
}

func (m *mockChapterProgressStore) GetChapterProgress(ctx context.Context, crewID, course string) (*game.CourseProgress, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func (m *mockChapterProgressStore) CreateChapterProgress(ctx context.Context, cp *game.CourseProgress) (*game.CourseProgress, error) {
	if m.err != nil {
		return nil, m.err
	}
	key := cp.FamilyID + "|" + cp.Course
...
func (m *mockChapterProgressStore) UpdateChapterProgress(ctx context.Context, crewID, course string, patch map[string]any) error {
	if m.err != nil {
		return m.err
	}
	key := crewID + "|" + course
...
func (m *mockChapterProgressStore) ListChapterProgressByCrew(ctx context.Context, crewID string) ([]game.CourseProgress, error) {
	if m.err != nil {
		return nil, m.err
	}
	result := make([]game.CourseProgress, 0)
...
func makeChapterDef(slug, journey, title string, order int) gamecontent.ChapterDefinition {
...
func TestListByRealm_ReturnsSortedChapters(t *testing.T) {
	gw := &mockChapterGateway{
		chapters: []gamecontent.ChapterDefinition{
			makeChapterDef("ch-b", "journey-1", "B", 2),
			makeChapterDef("ch-a", "journey-1", "A", 1),
			makeChapterDef("ch-c", "journey-2", "C", 1),
		},
	}
	svc := NewChapterService(newMockChapterProgressStore(), nil, gw, events.NopPublisher{})

	chapters, err := svc.ListByRealm(context.Background(), "journey-1")
	if err != nil {
...
func TestGet_ChapterLockedWhenNoProgress(t *testing.T) {
	gw := &mockChapterGateway{
		chapters: []gamecontent.ChapterDefinition{
			makeChapterDef("ch-1", "journey-1", "Course 1", 1),
		},
	}
	svc := NewChapterService(newMockChapterProgressStore(), nil, gw, events.NopPublisher{})
...
func TestGet_ChapterShowsActiveProgress(t *testing.T) {
	...
	store.progress["crew-1|ch-1"] = &game.CourseProgress{
		FamilyID:  "crew-1",
		Course: "ch-1",
		Journey:   "journey-1",
...
func TestListProgress_LockedWhenNoProgress(t *testing.T) {
	gw := &mockChapterGateway{
		chapters: []gamecontent.ChapterDefinition{
			makeChapterDef("ch-1", "journey-1", "Course 1", 1),
			makeChapterDef("ch-2", "journey-1", "Course 2", 2),
		},
	}
	svc := NewChapterService(newMockChapterProgressStore(), nil, gw, events.NopPublisher{})
...
func TestListProgress_ShowsExistingProgress(t *testing.T) {
	...
	store.progress["crew-1|ch-1"] = &game.CourseProgress{
		FamilyID: "crew-1", Course: "ch-1", Journey: "journey-1",
...
func TestGetProgressView_IdentifiesCurrentAndNext(t *testing.T) {
	gw := &mockChapterGateway{
		chapters: []gamecontent.ChapterDefinition{
			makeChapterDef("ch-1", "journey-1", "Ch 1", 1),
			makeChapterDef("ch-2", "journey-1", "Ch 2", 2),
			makeChapterDef("ch-3", "journey-1", "Ch 3", 3),
		},
	}
	store := newMockChapterProgressStore()
	completed := time.Now().UTC()
	...
	store.progress["crew-1|ch-2"] = &game.CourseProgress{
		FamilyID: "crew-1", Course: "ch-2", Journey: "journey-1",
		Status: ChapterStatusActive,
	}
	svc := NewChapterService(store, nil, gw, events.NopPublisher{})
	...
	if view.CurrentChapter == nil || view.CurrentChapter.Definition.Slug != "ch-2" {
		t.Errorf("expected current course ch-2, got %v", view.CurrentChapter)
	}
	if view.NextChapter == nil || view.NextChapter.Definition.Slug != "ch-3" {
		t.Errorf("expected next course ch-3, got %v", view.NextChapter)
	}
	if len(view.CompletedChapters) != 1 {
		t.Errorf("expected 1 completed course, got %d", len(view.CompletedChapters))
	}
...
func TestCheckCompletion_AllMandatoryDone(t *testing.T) {
	gw := &mockChapterGateway{
		...
		missions: []gamecontent.QuestDefinition{
			{Slug: "q1", Course: "ch-1", IsMandatory: true},
			{Slug: "q2", Course: "ch-1", IsMandatory: true},
		},
	}
	qs := &mockQuestStore{}
	svc := NewChapterService(newMockChapterProgressStore(), qs, gw, events.NopPublisher{})

	qs.missions = []game.Mission{
		{FamilyID: "c1", TemplateSlug: "q1", Status: "DONE"},
		{FamilyID: "c1", TemplateSlug: "q2", Status: "DONE"},
	}
	...
	if !done {
		t.Error("expected course complete, all mandatory missions done")
	}
}
...
func TestCheckCompletion_NotAllDone(t *testing.T) {
	...
	qs.missions = []game.Mission{
		{FamilyID: "c1", TemplateSlug: "q1", Status: "DONE"},
		{FamilyID: "c1", TemplateSlug: "q2", Status: "PENDING"},
	}
	...
	if done {
		t.Error("expected course not complete, q2 not done")
	}
}
...
func TestCheckCompletion_NoMandatoryQuests(t *testing.T) {
	gw := &mockChapterGateway{
		...
		missions: []gamecontent.QuestDefinition{
			{Slug: "q1", Course: "ch-1", IsMandatory: false},
		},
	}
	qs := &mockQuestStore{}
	svc := NewChapterService(newMockChapterProgressStore(), qs, gw, events.NopPublisher{})
	...
	if !done {
		t.Error("expected course complete when no mandatory missions")
	}
}
...
func TestMarkComplete_UpdatesStatusAndPublishes(t *testing.T) {
	...
	if ev.Course != "ch-1" || ev.FamilyID != "crew-1" || ev.Journey != "journey-1" {
...
func TestHandleQuestCompleted_AlreadyComplete_PreventsDuplicateEvent(t *testing.T) {
	...
	if len(*eventsPublished) != 0 {
		t.Errorf("expected 0 events published for already-complete course, got %d", len(*eventsPublished))
	}
}
...
func TestUnlockFirstChapter(t *testing.T) {
	...
	err := svc.UnlockFirstChapter(context.Background(), "crew-1", "journey-1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestUnlockFirstChapter_NoChapters(t *testing.T) {
	...
	err := svc.UnlockFirstChapter(context.Background(), "crew-1", "journey-1")
	if err != nil {
		t.Fatalf("expected no error for empty journey, got %v", err)
	}
}
...
func TestGetCurrentChapter_NoProgress(t *testing.T) {
	...
	if result == nil {
		t.Fatal("expected first course as fallback")
	}
...
func TestEnsureFirstChapterUnlocked_AlreadyHasProgress(t *testing.T) {
	...
	err := svc.EnsureFirstChapterUnlocked(context.Background(), "crew-1", "journey-1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
