...
var ErrChapterNotFound = errors.New("course not found")
var ErrChapterNotUnlocked = errors.New("course not unlocked")
...
type ChapterProgressView struct {
	CurrentChapter    *ChapterSummary  `json:"current_course,omitempty"`
	NextChapter       *ChapterSummary  `json:"next_course,omitempty"`
...
func (s *ChapterService) ListByRealm(ctx context.Context, journey string) ([]content.ChapterDefinition, error) {
...
func (s *ChapterService) ListProgress(ctx context.Context, crewID string) ([]ChapterSummary, error) {
	...
	progressMap := make(map[string]game.CourseProgress, len(progressRows))
...
func (s *ChapterService) UnlockFirstChapter(ctx context.Context, crewID, journey string) error {
	chapters, err := s.ListByRealm(ctx, journey)
	if err != nil {
		return err
	}
	if len(chapters) == 0 {
	...
	first := chapters[0]
	_, err = s.progress.CreateChapterProgress(ctx, &game.CourseProgress{
		FamilyID:  crewID,
		Course: first.Slug,
		Journey:   journey,
	...
	if err != nil {
		...
		return fmt.Errorf("create course progress: %w", err)
	}
	return nil
}

func (s *ChapterService) EnsureFirstChapterUnlocked(ctx context.Context, crewID, journey string) error {
	existing, err := s.progress.ListChapterProgressByCrew(ctx, crewID)
	if err == nil && len(existing) > 0 {
		return nil
	}
	return s.UnlockFirstChapter(ctx, crewID, journey)
}
...
func (s *ChapterService) unlockNextChapter(ctx context.Context, crewID, currentRealm, currentChapter string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("unlock next course: %w", err)
	}
	return nil
}
...
func (s *ChapterService) CheckCompletion(ctx context.Context, crewID, chapterSlug string) (bool, error) {
	...
	missions, err := s.content.ListQuests(ctx)
	if err != nil {
		return false, err
	}
	mandatory := make(map[string]bool)
	for _, q := range missions {
		if q.Course == chapterSlug && q.IsMandatory {
	...
	instances, err := s.missions.ListQuestByCrew(ctx, crewID)
	if err != nil {
		return false, err
	}
...
func (s *ChapterService) MarkComplete(ctx context.Context, crewID, chapterSlug, playerUID string) error {
	...
	if err := s.progress.UpdateChapterProgress(ctx, crewID, chapterSlug, patch); err != nil {
		return fmt.Errorf("mark course complete: %w", err)
	}
...
func (s *ChapterService) handleQuestCompleted(ctx context.Context, event events.Event) error {
	...
	course := e.Course
	if course == "" && e.TemplateSlug != "" {
		questDef, qErr := s.content.GetQuest(ctx, e.TemplateSlug)
		if qErr == nil && questDef != nil {
			course = questDef.Course
		}
	}
	if course == "" {
		return nil
	}
	complete, err := s.CheckCompletion(ctx, e.FamilyID, course)
	if err != nil {
		return err
	}
	if !complete {
	...
	cp, err := s.progress.GetChapterProgress(ctx, e.FamilyID, course)
	if err != nil || cp == nil {
		return nil
	}
	...
	if err := s.MarkComplete(ctx, e.FamilyID, course, e.PlayerUID); err != nil {
		return fmt.Errorf("complete course: %w", err)
	}

	return s.unlockNextChapter(ctx, e.FamilyID, e.Journey, course)
}
