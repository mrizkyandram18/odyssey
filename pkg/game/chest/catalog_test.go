...
func TestContentChestCatalog_Get_FromStore(t *testing.T) {
	store := &mockChestDefinitionStore{
		defs: []game.ChestDefinition{
			{ID: 1, Slug: "wooden-chest", Name: "Wooden Chest", Rarity: "COMMON", Icon: "📦", Description: "A chest", CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
		},
		entries: map[string][]game.DropTableEntry{
			"wooden-chest": {
				{ID: 1, ChestSlug: "wooden-chest", Rarity: "COMMON", Weight: 0.7, CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
				{ID: 2, ChestSlug: "wooden-chest", Rarity: "UNCOMMON", Weight: 0.3, CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
			},
		},
	}
	...
	if ct.Name != "Wooden Chest" {
		t.Errorf("expected Wooden Chest, got %s", ct.Name)
	}
...
func TestContentChestCatalog_ListAll_FromStore(t *testing.T) {
	store := &mockChestDefinitionStore{
		defs: []game.ChestDefinition{
			{ID: 1, Slug: "wooden-chest", Name: "Wooden Chest", Rarity: "COMMON", Icon: "📦", Description: "A chest", CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
			{ID: 2, Slug: "bronze-chest", Name: "Bronze Chest", Rarity: "UNCOMMON", Icon: "🟤", Description: "A sturdy chest", CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
		},
		entries: map[string][]game.DropTableEntry{
			"wooden-chest": {
				{ID: 1, ChestSlug: "wooden-chest", Rarity: "COMMON", Weight: 0.7, CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
			...
			"bronze-chest": {
				{ID: 2, ChestSlug: "bronze-chest", Rarity: "UNCOMMON", Weight: 0.5, CreatedAt: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)},
			},
		},
	}
	catalog := NewContentChestCatalog(store)

	chests := catalog.ListAll(context.Background())
	if len(chests) != 2 {
		t.Errorf("expected 2 chests, got %d", len(chests))
	}
}
...
func TestContentChestCatalog_ListAll_Fallback(t *testing.T) {
	...
	chests := catalog.ListAll(context.Background())
	if len(chests) == 0 {
		t.Error("expected fallback chests")
	}
}
...
func TestContentChestCatalog_ListAll_StoreError(t *testing.T) {
	...
	chests := catalog.ListAll(context.Background())
	if len(chests) == 0 {
		t.Error("expected fallback chests when store errors")
	}
}
...
func TestContentChestCatalog_NilStore(t *testing.T) {
	...
	chests := catalog.ListAll(context.Background())
	if len(chests) == 0 {
		t.Error("expected fallback chests when store is nil")
	}
}
...
func TestContentChestCatalog_ListAll_UsesFallbackWhenDropTableLoadFails(t *testing.T) {
	...
	if chests[0].Slug != "wooden-chest" {
		t.Errorf("expected wooden-chest fallback, got %s", chests[0].Slug)
	}
...
func TestContentChestCatalog_ListAll_LogsWarningWhenNoFallbackExists(t *testing.T) {
	...
	if len(chests) != 0 {
		t.Fatalf("expected 0 chests (skipped because no fallback), got %d", len(chests))
	}
