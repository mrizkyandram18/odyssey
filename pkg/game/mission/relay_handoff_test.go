...
// publisher, a two-member crew, and a relay quest with two pending exercises.
...
func makeRelayQuestStore() *mockQuestStore {
	...
	return &mockQuestStore{
		...
		exercises: map[int64][]game.Exercise{
			1: {
				{ID: 10, MissionID: 1, Status: "PENDING", AssignedTo: func() *string { s := "alice"; return &s }()},
				{ID: 11, MissionID: 1, Status: "PENDING", AssignedTo: &bobUID},
			},
		},
	}
}
...
func TestPublishRelayHandoff_EmittedAfterSuccessfulAssignment(t *testing.T) {
	...
	if e.MissionID != 1 {
		t.Errorf("MissionID: got %d, want 1", e.MissionID)
	}
}
...
func TestPublishRelayHandoff_NoEventOnQuestCompletion(t *testing.T) {
	...
	store := &mockQuestStore{
		missions: map[int64]*game.Mission{
			2: {
				ID: 2, FamilyID: "crew-1", TemplateSlug: "relay-quest", Title: "Relay Mission",
				Status: "ACTIVE", StartedAt: func() *time.Time { t := time.Now(); return &t }(),
			},
		},
		exercises: map[int64][]game.Exercise{
			2: {
				{ID: 20, MissionID: 2, Status: "DONE", CompletedBy: "alice"},
				{ID: 21, MissionID: 2, Status: "PENDING", AssignedTo: &bobUID},
			},
		},
	}
	...
	// Mission becomes DONE — questCompleted=true → no relay handoff emitted.
...
func TestPublishRelayHandoff_NoEventOnReplay(t *testing.T) {
	...
	store := &mockQuestStore{
		missions: map[int64]*game.Mission{
			3: {
				ID: 3, FamilyID: "crew-1", TemplateSlug: "relay-quest", Title: "Relay",
				Status: "ACTIVE", StartedAt: func() *time.Time { t := time.Now(); return &t }(),
			},
		},
		exercises: map[int64][]game.Exercise{
			3: {
				{ID: 30, MissionID: 3, Status: "DONE", CompletedBy: "alice"},
				{ID: 31, MissionID: 3, Status: "PENDING"},
			},
		},
	}
