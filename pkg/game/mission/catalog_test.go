...
func TestMVPCatalog_ExactlySixWhisperingWoods(t *testing.T) {
	slugs := MVPQuestSlugs()
	if len(slugs) != 6 {
		t.Fatalf("expected 6 MVP missions, got %d", len(slugs))
	}
	...
	for _, slug := range slugs {
		...
		if tpl.Journey != MVPRealm {
			t.Errorf("%s: journey want %s got %s", slug, MVPRealm, tpl.Journey)
		}
		if len(tpl.ChallengeDefs) < 2 {
			t.Errorf("%s: want at least 2 exercises, got %d", slug, len(tpl.ChallengeDefs))
		}
		types[tpl.Type]++
	}
