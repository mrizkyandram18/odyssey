...
func (m *concurrentDailyTurnStore) CreateDailyTurn(ctx context.Context, dt *game.DailyMission) (*game.DailyMission, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	for _, t := range m.turns {
		if t.UID == dt.UID && t.Date == dt.Date && t.MissionSlug == dt.MissionSlug {
...
func (m *concurrentDailyTurnStore) ListDailyTurns(ctx context.Context, uid string) ([]game.DailyMission, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	result := make([]game.DailyMission, len(m.turns))
	copy(result, m.turns)
	return result, nil
}
...
func TestConsumeDailyTurn_ConcurrentSameQuest_OnlyOneSucceeds(t *testing.T) {
	...
	results := make([]*game.DailyMission, 5)
	errs := make([]error, 5)
