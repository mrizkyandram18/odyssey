...
func (s *DailyTurnService) List(ctx context.Context, uid string) ([]game.DailyMission, error) {
...
func (s *DailyTurnService) ConsumeDailyTurn(ctx context.Context, uid string, date string, questSlug string) (*game.DailyMission, error) {
	completedCount := 0
	alreadyDone := false
	turns, err := s.store.ListDailyTurns(ctx, uid)
