...
func makeTurn(date string, completed bool) game.DailyMission {
...
func newService(turns []game.DailyMission, cfg *DailyTurnConfig) *DailyTurnService {
	if cfg == nil {
		cfg = &DailyTurnConfig{}
	}
	cfg = &DailyTurnConfig{
...
func TestIsAvailableToday_RespectsMaxTurnsPerDay(t *testing.T) {
	...
	store := &mockDailyTurnStore{
		turns: []game.DailyMission{
			{ID: 1, UID: "user-1", Date: today, MissionSlug: "morning-light", Completed: true, CreatedAt: time.Now().UTC()},
			{ID: 2, UID: "user-1", Date: today, MissionSlug: "riddle-of-the-stones", Completed: true, CreatedAt: time.Now().UTC()},
		},
	}
...
func TestConsumeDailyTurn_Success(t *testing.T) {
	...
	store := &mockDailyTurnStore{
		turns: []game.DailyMission{
			{ID: 1, UID: "user-1", Date: today, MissionSlug: "morning-light", Completed: false, CreatedAt: time.Now().UTC()},
		},
	}
	svc := NewDailyTurnService(store, &DailyTurnConfig{})
	...
	if turn.MissionSlug != "morning-light" {
		t.Errorf("expected mission_slug morning-light, got %s", turn.MissionSlug)
	}
}

func TestConsumeDailyTurn_NoRecord(t *testing.T) {
	today := TodayDate()
	store := &mockDailyTurnStore{turns: []game.DailyMission{}}
	svc := NewDailyTurnService(store, &DailyTurnConfig{})
	turn, err := svc.ConsumeDailyTurn(context.Background(), "user-1", today, "morning-light")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestDailyTurnAPIHandler_PublishesDailyTurnCompletedEvent(t *testing.T) {
	store := &mockDailyTurnStore{
		turns: []game.DailyMission{
			{ID: 1, UID: "user-1", Date: "2026-08-03", MissionSlug: "morning-light", Completed: false, CreatedAt: time.Now().UTC()},
		},
	}
	dts := NewDailyTurnService(store, &DailyTurnConfig{XP: 10, MaxTurnsPerDay: 3})

	userStore := &mockUserStoreForDailyTurn{player: &game.Player{UID: "user-1", FamilyID: "crew-1", Level: 1, XP: 0}}
...
func (m *mockBalanceStoreForDailyTurn) ListOverrides(ctx context.Context) ([]balance.Override, error) {
	return []balance.Override{
		{Key: "daily_mission_xp", Value: m.xpOverride},
	}, nil
}
