...
func buildHandlerWithPublisher(turns []game.DailyTurn, pub events.Publisher) *DailyTurnAPIHandler {
...
func TestDailyTurnPush_NoAssignment_NoEvent(t *testing.T) {
	...
	today := TodayDate()
	alreadyDone := game.DailyTurn{
		ID: 1, UID: "u1", Date: today, QuestSlug: "morning-light", Completed: true,
	}
	pub := &capturePub{}
	h := buildHandlerWithPublisher([]game.DailyTurn{alreadyDone}, pub)
