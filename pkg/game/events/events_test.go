...
func TestDispatcher_PublishToSubscribedHandler(t *testing.T) {
	...
	if ev.QuestID != 1 {
		t.Errorf("expected quest ID 1, got %d", ev.QuestID)
	}
}
...
func TestDispatcher_PublishToMultipleHandlers(t *testing.T) {
	...
	d.Publish(context.Background(), QuestCompletedEvent{CrewID: "c1"})
...
func TestDispatcher_NoHandlersForEventType(t *testing.T) {
	...
	d.Publish(context.Background(), ChapterCompletedEvent{CrewID: "c1"})
