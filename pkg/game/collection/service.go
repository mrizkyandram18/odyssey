...
func (s *RelicService) ListInventory(ctx context.Context, uid string) ([]InventoryItem, error) {
	...
	for _, pr := range playerRelics {
		...
		def := s.GetRelicDefinition(ctx, pr.CollectionSlug)
		if def == nil {
			continue
		}
...
func (s *RelicService) GetLatestRelic(ctx context.Context, uid string) (*InventoryItem, error) {
	...
	def := s.GetRelicDefinition(ctx, latest.CollectionSlug)
	if def == nil {
		return nil, ErrRelicNotFound
	}
	...
	return &InventoryItem{
		...
		CollectionSlug:    latest.CollectionSlug,
		Name:         def.Name,
		Description:  def.Description,
		Journey:        def.Journey,
		Rarity:       def.Rarity,
...
func (s *RelicService) GiftRelic(ctx context.Context, senderUID, recipientUID, relicSlug, crewID string) (*GiftResult, error) {
	...
	if recipient.FamilyID != crewID {
		return nil, ErrCrossCrewGift
	}

	// 3. Verify Collection Definition
	...
	if err != nil {
		if err == game.ErrNotFound {
			_, err = s.playerRel.CreatePlayerRelic(ctx, &game.PlayerRelic{
				UID:          recipientUID,
				CollectionSlug:    relicSlug,
				CollectionID:      senderRelic.CollectionID,
				OwnedCount:   1,
				IsNew:        true,
	...
	metaSent := fmt.Sprintf(`{"collection_slug":"%s","recipient_uid":"%s"}`, relicSlug, recipientUID)
	metaRecv := fmt.Sprintf(`{"collection_slug":"%s","sender_uid":"%s"}`, relicSlug, senderUID)
