...
func (m *mockPlayerRelicStore) GetPlayerRelic(ctx context.Context, uid, relicSlug string) (*game.PlayerRelic, error) {
	r, ok := m.collections[uid+"_"+relicSlug]
	if !ok {
		return nil, game.ErrNotFound
	}
	return r, nil
...
func (m *mockPlayerRelicStore) CreatePlayerRelic(ctx context.Context, item *game.PlayerRelic) (*game.PlayerRelic, error) {
	...
	m.collections[item.UID+"_"+item.CollectionSlug] = item
	return item, nil
}
...
func defaultStores() (*mockPlayerRelicStore, *mockUserStore, *mockLedgerStore) {
	prStore := &mockPlayerRelicStore{
		collections: map[string]*game.PlayerRelic{
			"sender1_ancient-compass": {UID: "sender1", CollectionSlug: "ancient-compass", OwnedCount: 2},
			"sender1_crystal-shard":   {UID: "sender1", CollectionSlug: "crystal-shard", OwnedCount: 0},
		},
	}
	uStore := &mockUserStore{
		users: map[string]*game.Player{
			"sender1": {UID: "sender1", FamilyID: "crewA", ExplorerName: "Sender"},
			"recv1":   {UID: "recv1", FamilyID: "crewA", ExplorerName: "Recv"},
			"recvB":   {UID: "recvB", FamilyID: "crewB", ExplorerName: "Recv B"},
		},
	}
...
func TestGiftRelic(t *testing.T) {
	...
	t.Run("HappyPath", func(t *testing.T) {
		...
		if prStore.collections["sender1_ancient-compass"].OwnedCount != 1 {
			t.Errorf("sender store count not decremented")
		}
		if prStore.collections["recv1_ancient-compass"].OwnedCount != 1 {
	...
	t.Run("SelfGift", func(t *testing.T) {
		...
		if prStore.collections["sender1_ancient-compass"].OwnedCount != 2 {
	...
	t.Run("SenderDecrementFailure_NoGiftResult", func(t *testing.T) {
		...
		if _, ok := prStore.collections["recv1_ancient-compass"]; ok {
