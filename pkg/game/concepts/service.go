...
var ErrLoreNotFound = errors.New("concept not found")
...
type LoreGateway interface {
	...
	ListLoreByChapter(ctx context.Context, course string) ([]content.LoreDefinition, error)
	ListLoreByRealm(ctx context.Context, journey string) ([]content.LoreDefinition, error)
}
...
func (s *LoreService) UnlockForChapter(ctx context.Context, crewID, course string) ([]LoreView, error) {
	loreDefs, err := s.content.ListLoreByChapter(ctx, course)
	if err != nil {
		return nil, fmt.Errorf("list concept by course: %w", err)
	}
	now := time.Now().UTC()
	...
	for _, ld := range loreDefs {
		...
		if err != nil {
			return nil, fmt.Errorf("unlock concept %s: %w", ld.Slug, err)
		}
...
func (h *ChapterCompletedHandler) Handle(ctx context.Context, event events.Event) error {
	...
	_, err := h.svc.UnlockForChapter(ctx, e.FamilyID, e.Course)
	return err
}
