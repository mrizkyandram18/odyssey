...
func (g *questGate) IsChapterUnlocked(ctx context.Context, crewID, chapter string) bool {
	cp, err := g.chapters.GetChapterProgress(ctx, crewID, chapter)
	if err != nil || cp == nil {
		return false
	}
...
func (g *questGate) IsRealmActive(ctx context.Context, crewID, realm string) bool {
	rp, err := g.realms.GetRealmProgress(ctx, crewID, realm)
	if err != nil || rp == nil {
		return false
	}
...
func (g *questGate) IsQuestCompleted(ctx context.Context, crewID, templateSlug string) bool {
	quests, err := g.quests.ListQuestByCrew(ctx, crewID)
	if err != nil {
		return false
	}
