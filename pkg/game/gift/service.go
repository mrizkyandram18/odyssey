...
func (s *ChestService) CreateChest(ctx context.Context, uid, chestSlug, source, rewardRelic string) (*game.Gift, error) {
	...
	created, err := s.gifts.CreateChest(ctx, ch)
	if err != nil {
...
// ListChests returns all gifts owned by a user.
func (s *ChestService) ListChests(ctx context.Context, uid string) ([]game.Gift, error) {
	return s.gifts.ListChestsByUser(ctx, uid)
}

// GetChest returns a single chest by ID, ensuring it belongs to the user.
func (s *ChestService) GetChest(ctx context.Context, chestID int64, uid string) (*game.Gift, error) {
	ch, err := s.gifts.GetChest(ctx, chestID)
	if err != nil {
...
func (s *ChestService) OpenChest(ctx context.Context, chestID int64, uid string) (*OpenResult, error) {
	...
	if ch.RewardRelic != "" {
		...
		rewards = s.engine.GenerateRewardsForChest(ctx, ch.GiftSlug, rewardCount)
	}
	...
	for i := range rewards {
		...
		existing, err := s.playerRel.GetPlayerRelic(ctx, uid, rewards[i].CollectionSlug)
		...
		if existing == nil {
			...
			newCount++
			def := relic.GetRelicDefinition(rewards[i].CollectionSlug)
			relicID := int64(0)
			if def != nil {
				relicID = def.ID
			}
			pr := &game.PlayerRelic{
				UID:          uid,
				CollectionSlug:    rewards[i].CollectionSlug,
				CollectionID:      relicID,
				OwnedCount:   1,
				IsNew:        true,
				DiscoveredAt: now,
				CreatedAt:    now,
			...
			if err := s.playerRel.UpdatePlayerRelic(ctx, uid, rewards[i].CollectionSlug, patch); err != nil {
				return nil, fmt.Errorf("update player relic: %w", err)
			}
		}

		instance := &game.Collection{
			UID:         uid,
			Code:        rewards[i].CollectionSlug,
			Name:        rewards[i].Name,
			Description: "",
			Journey:       "",
		...
		if def := relic.GetRelicDefinition(rewards[i].CollectionSlug); def != nil {
			instance.Description = def.Description
			instance.Journey = def.Journey
			instance.Image = def.Image
			instance.Concept = def.Concept
		}
		if _, err := s.collections.CreateRelic(ctx, instance); err != nil {
	...
	matched, err := s.gifts.UpdateChestIfMatch(ctx, chestID, false, chestPatch)
	if err != nil {
	...
	openedChest, _ := s.gifts.GetChest(ctx, chestID)
...
func ToChestView(ch game.Gift) ChestView {
	return ChestView{
		ID:          ch.ID,
		UID:         ch.UID,
		Source:      ch.Source,
...
func (h *QuestCompletedHandler) Handle(ctx context.Context, event events.Event) error {
	...
	existing, err := h.svc.gifts.ListChestsByUser(ctx, e.PlayerUID)
	if err == nil {
		for _, ch := range existing {
			if ch.Source == "QUEST" && ch.GiftSlug == qd.RewardChest && ch.UID == e.PlayerUID {
