...
type concurrentChestStore struct {
	mu     sync.Mutex
	gifts map[int64]*game.Gift
	nextID int64
}

func newConcurrentChestStore() *concurrentChestStore {
	return &concurrentChestStore{
		gifts: make(map[int64]*game.Gift),
	}
}

func (m *concurrentChestStore) CreateChest(ctx context.Context, ch *game.Gift) (*game.Gift, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.nextID++
	ch.ID = m.nextID
...
func (m *concurrentChestStore) GetChest(ctx context.Context, chestID int64) (*game.Gift, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	ch, ok := m.gifts[chestID]
	if !ok {
...
func (m *concurrentChestStore) ListChestsByUser(ctx context.Context, uid string) ([]game.Gift, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	result := make([]game.Gift, 0, len(m.gifts))
	for _, ch := range m.gifts {
		if ch.UID == uid {
...
func (m *concurrentRelicStore) CreateRelic(ctx context.Context, r *game.Collection) (*game.Collection, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.collections = append(m.collections, *r)
	return r, nil
}

func (m *concurrentRelicStore) GetRelic(ctx context.Context, relicID int64) (*game.Collection, error) {
	return nil, game.ErrNotFound
}

func (m *concurrentRelicStore) ListRelics(ctx context.Context) ([]game.Collection, error) {
	return nil, nil
}
...
func TestOpenChest_CompareAndSet_PreventsDoubleOpen(t *testing.T) {
	...
	if result == nil || !result.Gift.Opened {
...
func TestQuestCompletedHandler_NoDuplicateChestsOnReplay(t *testing.T) {
	...
	for _, ch := range gifts {
		if ch.Source == "QUEST" && ch.GiftSlug == "wooden-chest" {
			questChests++
		}
	}
	if questChests != 1 {
