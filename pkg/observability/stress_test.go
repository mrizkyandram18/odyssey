...
func TestObservability_Wrap_Concurrent(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		go func() {
			...
			for j := 0; j < perWorker; j++ {
				req := httptest.NewRequest(http.MethodPost, "/api/quests/complete", nil)
	...
	if snap.RequestCount["POST /api/quests/complete"] != total {
		t.Errorf("expected %d requests, got %d", total, snap.RequestCount["POST /api/quests/complete"])
	}
