...
export function StoryPage() {
    ...
    return (
      <div className="flex flex-col gap-4 p-4 pb-safe max-w-2xl mx-auto w-full">
        <div className="flex h-32 items-center justify-center">
          <p className="text-sm text-muted-foreground animate-pulse">Loading story…</p>
        </div>
      </div>
    )
  }
  ...
  if (error || !submission) {
    return (
      <div className="flex flex-col gap-4 p-4 pb-safe max-w-2xl mx-auto w-full">
        <h1 className="text-2xl font-bold">Story Page</h1>
        <div className="flex flex-col items-center gap-2 p-6">
          <p className="text-sm text-red-500">{error || 'Story not found'}</p>
  ...
  if (submission.kind !== 'DRAWING') {
    return (
      <div className="flex flex-col gap-4 p-4 pb-safe max-w-2xl mx-auto w-full">
        <h1 className="text-2xl font-bold">Story Page</h1>
        <div className="flex flex-col items-center gap-2 p-6">
          <p className="text-sm text-red-500">This submission is not a drawing.</p>
          <Link to="/gallery" className="text-sm text-primary underline">Back to Gallery</Link>
        </div>
      </div>
    )
    ...
    <div className="flex flex-col gap-4 p-4 pb-safe max-w-2xl mx-auto w-full">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-muted-foreground">
          Quest #{submission.quest_id} · Drawing
        </span>
