 ...
 *
 * State and validation ownership stays in useAdminConfig; sections receive
 * the hook result plus precomputed display numbers and render fields only.
 * No business logic, no API calls, no new state management here — except
 * strictly local UI toggles (tier inputs, advanced dropdown).
 */
export type EconomySettings = ReturnType<typeof useAdminConfig>
...
/**
 * Marks fields whose change immediately affects coin math or member access.
 * Visual distinction only — saving still goes through the single form POST.
 */
export const HighImpactBadge: React.FC = () => (
  <span
    title="Perubahan field ini langsung memengaruhi perhitungan koin atau akses anggota"
    className="ml-1.5 inline-flex items-center rounded-md bg-accent-gold/15 border border-accent-gold/30 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-accent-gold align-middle"
  >
    Dampak Tinggi
  </span>
)

export const EconomyHeader: React.FC<{ isOpen: boolean | null }> = ({ isOpen }) => (
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-border-subtle/60">
    ...
    {isOpen !== null && (
      ...
      >
        <span className={`w-2 h-2 rounded-full ${isOpen ? 'bg-status-success animate-pulse' : 'bg-text-secondary'}`} />
        {isOpen ? 'Pencairan Dibuka' : 'Pencairan Ditutup'}
      </span>
    )}
  </div>
)

export const CapFormulaHero: React.FC<{
  baseCapNum: number
  ceilingNum: number
  parsedTiers: ParsedTier[]
}> = ({ baseCapNum, ceilingNum, parsedTiers }) => (
...
export const MonthlyCapSection: React.FC<{
  settings: EconomySettings
  baseCapNum: number
  ceilingNum: number
  parsedTiers: ParsedTier[]
}> = ({ settings, baseCapNum, ceilingNum, parsedTiers }) => {
  const {
    monthlyCapInput,
    setMonthlyCapInput,
    maxCapCeilingInput,
  ...
  // Local UI state for the visual tier builder (inputs + inline error only).
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-1">
          <label htmlFor="input-monthly-cap" className="text-xs font-bold text-text-secondary flex items-center justify-between">
            <span>Batas Koin Bulanan Standar <span className="text-status-error">*</span><HighImpactBadge /></span>
        ...
        <div className="space-y-1">
          <label htmlFor="input-max-ceiling" className="text-xs font-bold text-text-secondary flex items-center justify-between">
            <span>Plafon Maksimum (Ceiling)<HighImpactBadge /></span>
...
export const ConversionTargetSection: React.FC<{
  settings: EconomySettings
  convRateNum: number
  targetRpNum: number
  targetCoinsCalc: number
}> = ({ settings, convRateNum, targetRpNum, targetCoinsCalc }) => {
  const {
    conversionRateInput,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label htmlFor="input-conversion-rate" className="text-xs font-bold text-text-secondary flex items-center justify-between">
            <span>Nilai 1 Koin (Rupiah) <span className="text-status-error">*</span><HighImpactBadge /></span>
            {convRateNum > 0 && (
...
export const RedemptionScheduleSection: React.FC<{ settings: EconomySettings }> = ({ settings }) => {
  const {
    startDayInput,
    setStartDayInput,
    endDayInput,
...
export const SecurityAdvancedSection: React.FC<{ settings: EconomySettings }> = ({ settings }) => {
  const {
    autoBlockInput,
    setAutoBlockInput,
    earningPeriodInput,
  ...
  // Local UI toggle for the advanced technical parameters dropdown.
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-start">
        <div className="space-y-1">
          <label htmlFor="input-auto-block" className="text-xs font-bold text-text-secondary">
            Batas Tandai Inaktif (Hari) <span className="text-status-error">*</span><HighImpactBadge />
          </label>
          <input
            id="input-auto-block"
            type="number"
            ...
            className="w-full p-2.5 rounded-xl bg-surface-elevated border border-border-subtle text-xs sm:text-sm font-bold text-text-primary focus:outline-none focus:border-accent-magic font-mono"
          />
          <p className="text-[10px] text-text-secondary">0 = nonaktif. Hanya penanda daftar perhatian — sistem tidak memblokir otomatis.</p>
        </div>
