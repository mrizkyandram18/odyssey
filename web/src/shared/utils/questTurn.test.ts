...
import { isMyRelayTurn } from './questTurn'
...
describe('isMyRelayTurn', () => {
  it('returns true when an active relay quest is assigned to me', () => {
  ...
  it('returns false for non-relay quest types', () => {
    expect(isMyRelayTurn({ ...relayActive, quest_type: 'SOLO' }, 'u1')).toBe(false)
    expect(isMyRelayTurn({ ...relayActive, quest_type: 'CREATIVE' }, 'u1')).toBe(false)
  })

  it('returns false for non-active quests', () => {
  ...
  it('handles null and undefined quest', () => {
