-- Gift & Collection definitions expansion (configurable catalogs)
...
CREATE TABLE IF NOT EXISTS odyssey_drop_tables (
  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  gift_slug TEXT NOT NULL REFERENCES odyssey_chest_definitions(slug) ON DELETE CASCADE,
  rarity TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_odyssey_drop_tables_chest_rarity
  ON odyssey_drop_tables (gift_slug, rarity);
...
CREATE INDEX IF NOT EXISTS idx_odyssey_relic_definitions_journey
  ON odyssey_relic_definitions (journey);
