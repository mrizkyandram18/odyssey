...
-- 1. Updates balance_configs for xp_per_level and max_new_missions_per_day
-- 2. Adds started_by to odyssey_missions to track user quest pacing
-- 3. Adds reward_relic mapping to quest definitions and gifts

-- 1. Balance Configs Update
INSERT INTO odyssey_balance_configs (key, value, updated_by)
VALUES
  ('xp_per_level', '500'::jsonb, 'system'),
  ('max_new_missions_per_day', '1'::jsonb, 'system')
...
-- Seed explicit reward_relic for existing missions
...
UPDATE odyssey_quest_definitions SET reward_relic = 'star-compass' WHERE slug = 'library-concept';
