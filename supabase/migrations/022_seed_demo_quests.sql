-- Migration 022: Phase 1 MVP — 6 Whispering Woods demo quests
-- --------------------------------------------------------------------------
-- Ensures demo-crew-1 has exactly 6 playable quests in whispering-woods with
...
-- ============================================================
-- 1. Unique index on challenges(quest_id, slug)
-- ============================================================
CREATE UNIQUE INDEX IF NOT EXISTS uq_odyssey_challenges_quest_slug
    ON odyssey_challenges (quest_id, slug);

-- ============================================================
-- 2. Quest type variety for the MVP six (definitions from 010)
-- ============================================================
...
--    Challenges cascade via FK ON DELETE CASCADE.
-- ============================================================
DELETE FROM odyssey_quests
WHERE crew_id = 'demo-crew-1'
...
-- ============================================================
-- 4. Drop stray challenges that do not belong to the MVP six
-- ============================================================
DELETE FROM odyssey_challenges
WHERE quest_id IN (
        SELECT id FROM odyssey_quests
        WHERE crew_id = 'demo-crew-1'
          AND template_slug NOT IN (
              'morning-light',
              'gather-herbs',
   ...
   OR (
        quest_id IN (SELECT id FROM odyssey_quests WHERE crew_id = 'demo-crew-1')
        AND slug NOT IN (
           'find-the-dew', 'morning-fact',
           'spot-the-green', 'herb-lore',
...
-- ============================================================
INSERT INTO odyssey_quests
    (id, crew_id, template_slug, title, chapter, status, started_at, completed_at)
VALUES
...
-- ============================================================
-- 6. Canonical challenges (2 each) — all PENDING for full playthrough
-- ============================================================
...
VALUES
    ...
    (102, 'herb-lore',      'Name one use for a common houseplant.',                                     'PENDING'),
...
ON CONFLICT (quest_id, slug) DO UPDATE SET
...
-- ============================================================
-- 7. Realm progress: only Whispering Woods is playable for MVP demo
-- ============================================================
...
ON CONFLICT (crew_id, realm) DO UPDATE SET
    status           = EXCLUDED.status,
    progress         = EXCLUDED.progress,
    last_unlocked_at = EXCLUDED.last_unlocked_at
WHERE odyssey_realm_progress.realm IN ('clockwork-city', 'starlit-library')
   OR (odyssey_realm_progress.realm = 'whispering-woods'
       AND odyssey_realm_progress.status = 'LOCKED');
...
VALUES ('schema_version', '022_mvp_six_quests')
