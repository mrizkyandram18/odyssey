...
-- Each member has a distinct avatar, has written journal memories after quests,
...
INSERT INTO odyssey_creative_submissions
  (id, quest_id, challenge_id, crew_id, author_uid, kind, content, status, created_at, updated_at)
VALUES
  -- Quest 101 (Morning Light) journal entries
  (1001, 101, (SELECT id FROM odyssey_challenges WHERE quest_id=101 AND slug='find-the-dew' LIMIT 1),
  ...
  (1002, 101, (SELECT id FROM odyssey_challenges WHERE quest_id=101 AND slug='morning-fact' LIMIT 1),
  ...
  -- Quest 102 (Gather Herbs) journal entry
  (1003, 102, (SELECT id FROM odyssey_challenges WHERE quest_id=102 AND slug='spot-the-green' LIMIT 1),
  ...
  (1004, 101, (SELECT id FROM odyssey_challenges WHERE quest_id=101 AND slug='find-the-dew' LIMIT 1),
   'demo-crew-1', 'demo-uid-1', 'STORY',
  ...
  (1005, 101, (SELECT id FROM odyssey_challenges WHERE quest_id=101 AND slug='morning-fact' LIMIT 1),
   'demo-crew-1', 'demo-uid-2', 'STORY',
...
-- ============================================================
-- 3. Reactions on Journal entries (new schema: crew_id, target_type, target_id, actor_uid)
-- Narrative: crew members react warmly and specifically to each other's memories.
-- Idempotent via unique constraint (crew_id, target_type, target_id, actor_uid).
-- ============================================================

INSERT INTO odyssey_reactions (crew_id, target_type, target_id, actor_uid, reaction_type, created_at)
VALUES
...
ON CONFLICT (crew_id, target_type, target_id, actor_uid)
DO UPDATE SET
