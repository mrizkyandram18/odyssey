...
// implementation (pkg/game/mission.QuestAPIHandler) composes QuestService with
// the progression and journey-progress collaborators.
...
//
//	/api/missions                      → list
//	/api/missions/<id>                 → get by id
//	/api/missions/<id>/start           → start quest (POST)
//	/api/missions/<id>/branch          → select narrative branch (POST)
//	/api/missions/<id>/exercises/<cid>/complete → complete challenge (POST)
...
func parseQuestPath(path string) routeInfo {
	...
	if !strings.HasPrefix(trimmed, "api/missions") {
		return routeInfo{}
	}
	rest := strings.TrimPrefix(trimmed, "api/missions")
	...
	case len(parts) == 4 && parts[1] == "exercises" && parts[3] == "complete":
...
func handleList(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims) {
	missions, err := handler.List(r.Context(), claims.FamilyID)
	if err != nil {
		shared.WriteJSONError(w, "failed to load missions", http.StatusInternalServerError)
		return
	}
	shared.WriteJSON(w, http.StatusOK, missions)
}

func handleListAvailable(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims) {
	missions, err := handler.ListAvailable(r.Context(), claims.FamilyID, claims.UID)
	if err != nil {
		shared.WriteJSONError(w, "failed to load available missions", http.StatusInternalServerError)
		return
	}
	shared.WriteJSON(w, http.StatusOK, missions)
}
...
func handleGetByID(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims, idStr string) {
	...
	q, err := handler.GetByCrewAndID(r.Context(), questID, claims.FamilyID)
	if err != nil {
		if isNotFound(err) {
...
func handleStart(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims, idStr string) {
	...
	if err := handler.StartQuest(r.Context(), questID, claims.FamilyID, claims.UID); err != nil {
		if isNotFound(err) {
			shared.WriteJSONError(w, "quest not found", http.StatusNotFound)
			return
		}
...
func handleCompleteChallenge(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims, idStr, cidStr string) {
	...
	result, err := handler.CompleteChallenge(r.Context(), questID, challengeID, claims.FamilyID, claims.UID, req.Answer)
	if err != nil {
		if isNotFound(err) {
			shared.WriteJSONError(w, "quest not found", http.StatusNotFound)
			return
...
func handleSelectBranch(w http.ResponseWriter, r *http.Request, claims *auth.SessionClaims, idStr string) {
	...
	result, err := handler.SelectBranch(r.Context(), questID, claims.FamilyID, req.Branch)
	if err != nil {
