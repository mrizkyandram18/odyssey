...
type mockRelicService struct {
	collections    []gamerelic.RelicDefinition
...
func TestRelicsHandler_InventorySuccess(t *testing.T) {
	...
	Setup(&mockRelicService{
		inventory: []gamerelic.InventoryItem{
			{CollectionSlug: "ancient-compass", Name: "Ancient Compass", Rarity: "COMMON", Image: "🧭", OwnedCount: 1, IsNew: true, DiscoveredAt: now, CreatedAt: now},
		},
	})
	...
	req := httptest.NewRequest(http.MethodGet, "/api/collections/inventory", nil)
...
func TestRelicsHandler_GetRelicSuccess(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/collections/ancient-compass", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	w := httptest.NewRecorder()
	mw.RequireAuth(Handler)(w, req)
...
func TestRelicsHandler_GiftSuccess(t *testing.T) {
	...
	body := `{"recipient_uid":"recipient-uid","collection_slug":"ancient-compass"}`
	req := httptest.NewRequest(http.MethodPost, "/api/collections/gift", strings.NewReader(body))
	req.Header.Set("Authorization", "Bearer "+token)
	w := httptest.NewRecorder()
	mw.RequireAuth(Handler)(w, req)
